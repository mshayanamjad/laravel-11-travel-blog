...
<section class="video-section">
    ...
    <div class="row">
        ...
        <div><img src="{{ asset('front-assets/images/slide-1.jpg') }}" /></div>
        <div><img src="{{ asset('front-assets/images/slide-2.jpg') }}" /></div>
        <div><img src="{{ asset('front-assets/images/slide-3.webp') }}" /></div>
        <div><img src="{{ asset('front-assets/images/slide-4.webp') }}" /></div>
        </div>
        <!--  Image slider Start Here -->
        <div class="slider-content">
        <h1 class="h1-title">EXPLORE. DREAM. DISCOVER.</h1>
        <p class="slider-text">
            This is a world travel blog featuring beautiful destinations, new
            experiences, and hidden places around the globe. <br /><br />
            Please tag along!
        </p>
        <a class="explore-btn" href="{{ route('front.blog') }}">START EXPLORING</a>
        </div>
    </div>
    </div>
</section>
<!--  Section 1 end Here -->
<section class="shape-divider p-0">
...
<!--  Section 2 end Here -->
<section class="home-about bg-white py-md-5">
    <div class="container pt-0 pt-md-5">
    <div class="row">
        <div class="col-md-6 d-flex align-items-center">
        <div class="about-img">
            <img class="img-opacity" src="{{ asset('front-assets/images/asset 13.jpeg') }}" alt="" />
        </div>
        </div>
        <div class="col-md-6 pt-5 p-md-0">
            <div class="about-content">
                <h2 class="h2-title">Thanks For Looking!</h2>
                <p class="about-text">
                I'm David Leiter, the guy behind this website. I'm an American
                who's been traveling the world full time for 8 years now.<br /><br />
                I started this travel blog in 2019 to document my own
                international trips, share my photos, and help others learn how
                to travel the world and find some really good spots off the
                beaten path.<br /><br />
                I'm currently based in Bali, Indonesia, where I met my wife
                Intan, who’s a Bali local. Now she joins me on these wild and
                crazy adventures too.<br /><br />All of the writing and pictures
                on this site come from my own personal experience.<br /><br />I'm
                currently based in Bali, Indonesia, where I met my wife Intan,
                who’s a Bali local. Now she joins me on these adventures too.<br /><br />Together,
                we've done some bucket list hikes, climbed active volcanoes,
                seen exotic wildlife, and visited some spectacular castles,
                temples, and monuments around the world.<br /><br />I've worked
                with and been featured by BBC Travel, NBC News, Time, and other
                companies.<br /><br />I haven’t been everywhere, but it’s on my
                list. I hope this world travel blog can help and inspire you in
                your own journeys as well!
                </p>
                <a class="about-btn" href="{{ route('front.about') }}">
                    <span>Read More</span>
...
<section class="travel-guide posts py-3 py-md-5 bg-white">
    <div class="container">
    <div class="row">
        ...
        <div class="col-md-12 grid-3">
            @foreach($latestPosts as $latestPost)
                <div class="post-container text-center">
                    <div class="inner-container">
                    <figure class="featured-image position-relative">
                        <a href="{{ route('front.singlePost', $latestPost->slug) }}" class="featured-image-link">
                        <img
                            class="img-opacity"
                            src="{{ asset('uploads/post-images/'. $latestPost->image) }}"
                        />
                        </a>
                        <div class="fav-icon">
                            @if (favPosts()->contains('post_id', $latestPost->id))
                                <a onclick="addToFav({{ $latestPost->id }})" href="javascript:void(0);">
                                    <i class="fa-solid fa-heart" style="font-size: 20px; color:#ff8a65;"></i>
                                </a>
                            @else
                                <a onclick="addToFav({{ $latestPost->id }})" href="javascript:void(0);">
                    ...
                    <div class="post-content">
                        <h4 class="post-title">
                        <a href="{{ route('front.singlePost', $latestPost->slug) }}" class="post-link">
                            {{ $latestPost->title }}
                        </a>
                        </h4>
                    </div>
...
<!-- Hikes Section -->
<section class="hikes posts py-3 py-md-5 bg-white">
    <div class="container pb-5">
    <div class="row">
        <div class="styled-title">
        <h2>Hikes</h2>
        </div>
        <p class="about-text text-center">Bucket list hiking guides</p>
        <div class="col-md-12 grid-3">
                
            @foreach($hikePosts as $hikePost)
                <div class="post-container text-center">
                    <div class="inner-container">
                    <figure class="featured-image position-relative">
                        <a href="{{ route('front.singlePost', $hikePost->slug) }}" class="featured-image-link">
                        <img
                            class="img-opacity"
                            src="{{ asset('uploads/post-images/'. $hikePost->image) }}"
                        />
                        </a>
                        <div class="fav-icon">
                            @if (favPosts()->contains('post_id', $hikePost->id))
                                <a onclick="addToFav({{ $hikePost->id }})" href="javascript:void(0);">
                                    <i class="fa-solid fa-heart" style="font-size: 20px; color:#ff8a65;"></i>
                                </a>
                            @else
                                <a onclick="addToFav({{ $hikePost->id }})" href="javascript:void(0);">
                    ...
                    <div class="post-content">
                        <h4 class="post-title">
                        <a href="{{ route('front.singlePost', $hikePost->slug) }}" class="post-link">
                            {{ $hikePost->title }}
                        </a>
                        </h4>
                    </div>
