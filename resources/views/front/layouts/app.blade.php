...
<html lang="en">
  <head>
    ...
    <link rel="icon" href="{{ asset('front-assets/images/favicon_512.webp') }}">

    <!-- Bootstrap -->
    <link rel="stylesheet" href="{{ asset('front-assets/bootstrap-5.0.2/css/bootstrap.min.css') }}" />

    <!-- FontAwesome -->
    <link rel="stylesheet" href="{{ asset('front-assets/fontawesome-free-6.2.1/css/all.min.css') }}" />

    <!-- jQuery -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css"/>
    <link rel="stylesheet" href="https://code.jquery.com/ui/1.14.1/themes/base/jquery-ui.css">
    <script src="https://code.jquery.com/jquery-3.7.1.js"></script> <!-- Latest jQuery version -->
    <script src="https://code.jquery.com/ui/1.14.1/jquery-ui.js"></script> <!-- Latest jQuery UI version -->
    <script src="https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js"></script> <!-- Slick Slider -->
    <script src="{{ asset('front-assets/fontawesome-free-6.2.1/js/all.min.js') }}"></script>
    <script src="{{ asset('front-assets/bootstrap-5.0.2/js/bootstrap.min.js') }}"></script>
    ...
    <!-- Open Sans Condensed -->
    <link
      href="https://fonts.googleapis.com/css2?family=Open+Sans+Condensed:wght@300;400;600;700&display=swap"
      rel="stylesheet"
    />
    ...
    <!-- Oswald -->
    <link
      href="https://fonts.googleapis.com/css2?family=Oswald:wght@400;500;600;700&800&display=swap"
      rel="stylesheet"
    />

    <!-- Philosopher -->
    <link
      href="https://fonts.googleapis.com/css2?family=Philosopher:wght@400;500;600;700;800&display=swap"
      rel="stylesheet"
    />
    ...
    <title>@yield('title') • How To Travel The World</title>
  </head>
  <body>
    <header class="header" id="header">
      <div class="container">
        <div class="d-flex align-items-center">
          <div class="col-10 col-md-4 p-0 logo-cont">
            <div class="logo">
              <a href="{{ route('front.home') }}">
                <img src="{{ asset('front-assets/images/asset 0.jpeg') }}" alt="Logo" />
              </a>
              <!-- Added alt attribute -->
            </div>
          </div>
          <div class="main-menu col-2 col-md-8 p-0 d-flex align-items-center justify-content-between">
            <div class="hamburger" id="hamburger" role="button" tabindex="0">
              <i class="fa-solid fa-bars"></i>
            </div>
            <nav class="navigation" id="nav-menu">
              <ul class="nav-list d-flex justify-content-start">
                <div class="close-menu" id="close-menu" role="button" tabindex="0">
                  <i class="fa-solid fa-xmark"></i>
                </div>
                <li><a class="nav-link" href="{{ route('front.home') }}">Home</a></li>
                <li><a class="nav-link" href="{{ route('front.blog') }}">Blog</a></li>
                <li>
                  <a class="nav-link sub-categories" href="javascript:void(0);">Categories</a>
                  <form method="get" action="{{ route('front.blog') }}">
                    <ul class="dropdown">
                      @if (getCategories()->isNotEmpty())
                        @foreach (getCategories() as $category)
                          @if ($category->sub_categories->isEmpty())
                            <li>
                              <!-- {{-- <a class="dropdown-link" href="{{ route('front.blog.category', $category->id) }}"> --}} -->
                              <a class="dropdown-link" href="{{ route('front.blog', ['category' => $category->id]) }}">
                                {{ $category->name }}
                            </a>
                          ...
                          @else
                            {{-- Add Sub Category Code Here --}}
                            <li class="super-sub-category">
                              <a class="dropdown-link  d-flex justify-content-between align-items-center" href="{{ route('front.blog', ['category' => $category->id]) }}">
                                {{ $category->name }}
                              ...
                              <ul class="sub-dropdown">
                                ...
                                <li>
                                  <a class="dropdown-link" href="{{ route('front.blog', ['sub_category' => $sub_category->id]) }}">{{ $sub_category->name }}</a>
                                </li>
                                @endforeach
                              </ul>
                            </li>
                ...
                <li>
                  <a class="nav-link sub-categories" href="javascript:void(0);">Gallery
                  </a>
                  <ul class="dropdown">
                    <li>
                      <a class="dropdown-link" href="{{ route('front.gallery') }}">Image Gallery</a>
                    </li>
                    <li>
                      <a class="dropdown-link" href="{{ route('front.travelGear') }}">My Camera Gear</a>
                    </li>
                  </ul>
                </li>
                <li><a class="nav-link" href="{{ route('front.about') }}">About</a></li>
                <li><a class="nav-link" href="{{ route('front.contact') }}">Contact</a></li>
                <li><a class="nav-link my-account d-none" href="{{ route('front.profile') }}">My Account</a></li>
              </ul>
            </nav>
            <div class="search-icon" aria-label="Search">
              <form action="{{ route('front.blog') }}" method="get" class="w-100">
                <input type="text" value="{{ Request::get('keyword') }}" name="keyword" id="search" placeholder="Search Posts Here..."/>
              </form>
              <img src="{{ asset('front-assets/images/search.svg') }}" id="search_icon" />
              <img src="{{ asset('front-assets/images/cancel.svg') }}" id="search_close" style="display: none; cursor: pointer" />
            ...
            <div class="profile-icon ms-3">
              <a href="{{ route('front.profile') }}">
                @if (auth()->user() && auth()->user()->image != null) 
                  <img src="{{ asset('uploads/profile-pic/' . auth()->user()->image) }}" alt="Profile Image" style="width: 30px; height: 30px; border-radius: 50%" />
                @else
                  <i class="fa-regular fa-circle-user" style="color: #adadad; font-size: 22px"></i>
                @endif
              </a>
          </div>
        </div>
    ...
    <footer class="footer" id="footer">
          ...
          <div class="container">
            <div class="row">
              ...
              <div class="icon-box">
                ...
                <p class="box-text">
                  Feel free to <a href="#">contact me</a> if you have travel
                  questions, comments, or suggestions.<br /><br />I'll try to get
                  back to you!
                </p>
        ...
        <section class="newsletter py-md-5" style="background-color: #f7f7f7">
          <div class="container">
            <div class="row">
              <h2 class="newsletter-title">Newsletter</h2>
              <p class="newsletter-text">
                Subscribe to my newsletter for the latest blog posts, tips, &
                travel guides. Let's stay updated!
              </p>
              <form action="{{ route('front.subscribe') }}" method="post" id="newsletter-form">
                @csrf
                <div class="form-container d-flex align-items-center justify-content-center">
                  <div class="form-row col-md-9 me-3">
                    <input type="email" name="email" id="email" placeholder="Email..."
                    />
                  </div>
                  <div class="form-row-btn col-md-3 text-end">
          ...
          <div class="container">
            <div class="div row">
              <div class="social-icons-container">
                <ul
                  class="icon-list d-flex align-items-center justify-content-center gap-5 list-unstyled"
                >
                  <a href="#" target="_blank" class="icon-list-item">
                  ...
                  <a href="mailto:mianshayan201@gmail.com" target="_blank" class="icon-list-item">
                    <i class="fa-solid fa-envelope"></i>
                  </a>
                </ul>
              </div>
              <div class="footer-inner-sec text-center">
                <p class="copyright">© The World Travel Guy 2024</p>
                <div class="footer-menu">
                  <a href="{{ route('front.profile') }}" class="footer-menu-item">My Account</a>
                  <span>•</span>
                  <a href="{{ route('front.login') }}" class="footer-menu-item">Login</a>
                  <span>•</span>
                  <a href="{{ route('front.register') }}" class="footer-menu-item">Registration</a>
                </div>
                <p class="copyright">
                  Some of the pages on my travel blog contain affiliate links.
                  Whenever you buy something through one of these links, I get a
                  small commission at no extra cost to you. As an affiliate, I
                  only recommend products and services that I feel are high
                  quality and helpful to my readers. Thanks for your support.
                </p>
              </div>
            </div>
          </div>
      ...
      <div class="modal-dialog w-100">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a href="{{ route('front.favPost') }}" class="btn btn-primary">Go to Favourite Posts</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <script>
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.status == true) {
              ...
              $('#addFavModel .modal-body').html(response.message);
              $('#addFavModel').css('display', 'flex').hide().slideDown('slow');
      ...
      jQuery(document).ready(function($) {
          $('.close').click(function() {
              $('#addFavModel').slideUp('slow'); // Use 'fast' for a quick slide-up effect
          });
